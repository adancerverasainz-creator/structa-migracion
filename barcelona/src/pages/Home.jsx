...
export default function Home() {
  ...
  // Ingresos por método de pago (incluye pagos de jugadores, pagos generales y pagos de torneos)
  ...
  const allPaymentsThisMonth = [...paymentsThisMonth, ...generalPaymentsThisMonth, ...monthTournamentPayments];
  ...
  const totalCard = cardPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
  const totalTransfer = transferPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  const cashTournamentPayments = monthTournamentPayments.filter(p => p.payment_method === 'efectivo').reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  const cardTournamentPayments = monthTournamentPayments.filter(p => p.payment_method === 'tarjeta').reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  const transferTournamentPayments = monthTournamentPayments.filter(p => p.payment_method === 'transferencia').reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  const totalBBVA = bbvaPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
  const totalMP = mpPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
  const totalNU = nuPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
  const totalOpenBank = openBankPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
  const totalMercadoPagoBIA = mercadoPagoBIAPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  const getBankBreakdown = (bankName) => {
    ...
    return {
      ...
      totalTournament: tournamentPaymentsBank.reduce((sum, p) => sum + (p.amount || 0), 0),
    };
  };
            ...
            <Tabs defaultValue="total" className="space-y-3">
              ...
              <TabsContent value="efectivo" className="space-y-2">
                ...
                <div className="flex justify-between items-center p-3 bg-emerald-100 rounded border-2 border-emerald-300">
                  ...
                  <span className="text-lg font-bold text-emerald-600">{formatCurrency(totalCash + cashTournamentPayments)}</span>
                </div>
              </TabsContent>
              ...
              <TabsContent value="tarjeta" className="space-y-2">
                ...
                <div className="flex justify-between items-center p-3 bg-blue-100 rounded border-2 border-blue-300">
                  ...
                  <span className="text-lg font-bold text-blue-600">{formatCurrency(totalCard + cardTournamentPayments)}</span>
                </div>
              </TabsContent>
              ...
              <TabsContent value="transferencia" className="space-y-2">
                ...
                <div className="flex justify-between items-center p-3 bg-purple-100 rounded border-2 border-purple-300">
                  ...
                  <span className="text-lg font-bold text-purple-600">{formatCurrency(totalTransfer + transferTournamentPayments)}</span>
                </div>
              </TabsContent>
            </Tabs>
          </CardContent>
